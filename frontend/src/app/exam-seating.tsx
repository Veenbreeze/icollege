...
import { seatingInfo as s } from '@/data/mock';
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.header}>
          ...
          <View style={{ flex: 1, marginLeft: spacing.md }}>
            <Text style={styles.title}>Exam Seating</Text>
            <Text style={styles.subtitle}>Find your seat</Text>
          </View>
        </View>
        ...
        <Card style={styles.infoCard}>
          <View style={styles.infoTop}>
            <View>
              ...
              <Text style={styles.examCode}>{s.code} · {s.date}</Text>
            </View>
            <View style={styles.regBadge}>
              <Text style={styles.regText}>{s.regNumber}</Text>
            </View>
          </View>
          ...
          <View style={styles.detailRow}>
            <Detail icon="albums-outline" label="Row" value={s.row} />
            <Detail icon="grid-outline" label="Seat" value={s.seatNo} />
          </View>
        </Card>
            ...
            {Array.from({ length: s.rows }).map((_, r) => (
              <View key={r} style={styles.gridRow}>
                ...
                {Array.from({ length: s.cols }).map((_, c) => {
                  const mine = r === s.myRow && c === s.myCol;
                  // scatter a few "occupied" seats for realism
                  const occupied = (r * s.cols + c) % 3 === 0 && !mine;
                  return (
                    <View
                      key={c}
                      style={[
...
const styles = themedStyles((colors) => ({
  ...
  infoCard: { marginHorizontal: spacing.lg, marginTop: spacing.lg },
  infoTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: spacing.lg },
  examName: { fontSize: 17, fontWeight: '800', color: colors.text },
  examCode: { fontSize: 12.5, color: colors.textSecondary, marginTop: 2 },
  regBadge: { backgroundColor: colors.primarySoft, paddingHorizontal: spacing.md, paddingVertical: 6, borderRadius: radii.pill },
  regText: { fontSize: 11, fontWeight: '700', color: colors.primary },
  detailRow: { flexDirection: 'row', gap: spacing.md, marginTop: spacing.sm },
  detail: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: spacing.sm },
  detailIcon: { width: 34, height: 34, borderRadius: radii.sm, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center' },
  ...
  mapCard: { marginHorizontal: spacing.lg },
  podium: { alignSelf: 'center', flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: colors.surfaceMuted, paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: radii.pill, marginBottom: spacing.lg },
  podiumText: { fontSize: 12, fontWeight: '600', color: colors.textSecondary },

  grid: { gap: spacing.sm },
  gridRow: { flexDirection: 'row', alignItems: 'center', gap: 6, justifyContent: 'center' },
  rowLabel: { width: 22, fontSize: 11, color: colors.textMuted, fontWeight: '600' },
  seat: { flex: 1, aspectRatio: 1, maxWidth: 34, borderRadius: 7, backgroundColor: colors.surfaceMuted, alignItems: 'center', justifyContent: 'center' },
  seatOccupied: { backgroundColor: colors.border },
  seatMine: { backgroundColor: colors.primary, ...shadow.soft, shadowColor: colors.primary },
  ...
  tipText: { flex: 1, fontSize: 12.5, color: colors.textSecondary, lineHeight: 18 },
}));
