...
import { chambers } from '@/data/mock';
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
        ...
        <Text style={styles.sectionTitle}>Your Chambers</Text>
        {chambers.map((c) => {
          const t = chamberThemes[c.theme] ?? chamberThemes.economics;
          return (
            <Pressable key={c.id} onPress={() => router.push(`/chamber/${c.id}` as any)}>
              <Card style={styles.row}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={styles.metaRow}>
                    ...
                    <Text style={styles.meta}>{c.members}</Text>
                  </View>
                </View>
...
const styles = themedStyles((colors) => ({
  ...
  clubsIcon: { width: 44, height: 44, borderRadius: radii.md, backgroundColor: 'rgba(255,255,255,0.22)', alignItems: 'center', justifyContent: 'center' },
  clubsTitle: { fontSize: 16, fontWeight: '800', color: colors.white },
  clubsSub: { fontSize: 12, color: 'rgba(255,255,255,0.9)', marginTop: 1 },
  ...
  row: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, marginHorizontal: spacing.lg, marginTop: spacing.md },
  icon: { width: 54, height: 54, borderRadius: radii.lg, alignItems: 'center', justifyContent: 'center' },
