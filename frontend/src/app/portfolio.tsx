...
import { portfolio as p } from '@/data/mock';
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.identity}>
          <Avatar initials={p.initials} size={84} ring />
          ...
          {p.experience.map((e) => (
            <View key={e.id} style={styles.timelineItem}>
              ...
              <View style={{ flex: 1, paddingBottom: spacing.lg }}>
                ...
                <Text style={styles.itemSub}>{e.org} · {e.period}</Text>
          ...
          {p.projectsList.map((pr) => (
            <Card key={pr.id} style={styles.rowCard}>
              <View style={[styles.rowIcon, { backgroundColor: colors.surfaceMuted }]}>
                <Text style={{ fontSize: 22 }}>{pr.emoji}</Text>
              </View>
              <View style={{ flex: 1 }}>
          ...
          {p.certifications.map((c) => (
            <Card key={c.id} style={styles.rowCard}>
              <View style={[styles.rowIcon, { backgroundColor: colors.blueSoft }]}>
                <Ionicons name="ribbon-outline" size={20} color={c.color} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.itemTitle}>{c.name}</Text>
                <Text style={styles.itemSub}>{c.issuer} · {c.year}</Text>
              </View>
          ...
          {p.education.map((ed) => (
            <Card key={ed.id} style={styles.rowCard}>
              <View style={[styles.rowIcon, { backgroundColor: colors.primarySoft }]}>
                <Ionicons name="school-outline" size={20} color={ed.color} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.itemTitle}>{ed.school}</Text>
                <Text style={styles.itemSub}>{ed.degree} · {ed.period}</Text>
              </View>
            </Card>
          ))}
        </Section>
...
const styles = themedStyles((colors) => ({
  ...
  identity: { alignItems: 'center', marginTop: spacing.lg, paddingHorizontal: spacing.lg },
  name: { fontSize: 21, fontWeight: '800', color: colors.text, marginTop: spacing.md },
  headline: { fontSize: 13.5, color: colors.textSecondary, marginTop: 3, textAlign: 'center' },
  locRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 6 },
  location: { fontSize: 12.5, color: colors.textMuted },
  openBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: colors.greenSoft, paddingHorizontal: spacing.md, paddingVertical: 6, borderRadius: radii.pill, marginTop: spacing.md },
  openDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: colors.green },
  openText: { fontSize: 12, fontWeight: '700', color: colors.green },
  ...
  statLabel: { fontSize: 10.5, color: colors.textMuted, marginTop: 2, textAlign: 'center' },
  vDiv: { width: 1, height: 34, backgroundColor: colors.border },
  ...
  skillTag: { backgroundColor: colors.primarySoft, paddingHorizontal: spacing.md, paddingVertical: 7, borderRadius: radii.sm },
  skillText: { fontSize: 12.5, color: colors.primary, fontWeight: '600' },
  ...
  itemSub: { fontSize: 12.5, color: colors.textMuted, marginTop: 2 },
  itemDesc: { fontSize: 13, color: colors.textSecondary, marginTop: 4, lineHeight: 19 },

  rowCard: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, marginBottom: spacing.sm },
  rowIcon: { width: 44, height: 44, borderRadius: radii.md, alignItems: 'center', justifyContent: 'center' },
  result: { fontSize: 12.5, fontWeight: '700', color: colors.text },
}));
