...
import { messagesPreview } from '@/data/mock';
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
        {messagesPreview.map((m) => (
          <Pressable key={m.id} style={styles.row} onPress={() => router.push(`/chat/${m.id}` as any)}>
            <Avatar initials={m.initials} size={52} color={m.color} />
            <View style={{ flex: 1 }}>
...
const styles = themedStyles((colors) => ({
  ...
  searchBar: {
    ...
    marginHorizontal: spacing.lg, marginTop: spacing.lg, paddingHorizontal: spacing.lg, height: 48, borderRadius: radii.lg, ...shadow.soft,
  },
  ...
  last: { fontSize: 13, color: colors.textSecondary, flex: 1, marginRight: spacing.sm },
  unread: { backgroundColor: colors.primary, minWidth: 20, height: 20, borderRadius: 10, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 6 },
